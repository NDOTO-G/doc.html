...
verifiers are the sealed reference readers shipped beside it, tools/verify.mjs
and tools/verify.py; SPEC.md is the normative text they implement.
...
def _section_inner_bytes(raw: bytes) -> dict[str, bytes]:
    ...
    Mirrors the sealed reference readers (tools/verify.mjs, tools/verify.py):
    the span is everything between the opening tag's '>' and the matching
    '</section>', untouched. First occurrence wins on duplicate ids (duplicates
    are a conformance failure the readers themselves refuse).
    """
