...
Vows are promises to the world. They name what this format will never do, and what it will always do. Each vow defends a specific failure mode — a way the format could betray the [mission](MISSION.md) — and names the mechanism by which the promise is kept.
...
- **How defended:** every `doc.html` includes a `format-spec` section in its body. The [`doc.html`](doc.html) beside this file is the v0.3 reference; [`SPEC.md`](SPEC.md) is the same definition, standalone.
...
- **How defended:** [`verify.mjs`](verify.mjs) and [`verify.py`](verify.py) re-hash every section against its witness, at build time and at any later moment. Any reader can re-run them — or re-derive the check from the in-band spec with no tool at all.
...
- **How defended:** the artifact is self-sufficient. The bundled verifiers ([`verify.mjs`](verify.mjs), [`verify.py`](verify.py)) and builder ([`build-doc.mjs`](build-doc.mjs)) are conveniences for authors and auditors, never requirements for reading.
...
- **How defended:** the document carries an `about` section naming its origin, license, and build date — in prose, the only place the spec puts build metadata. This repo's reference [`doc.html`](doc.html) states: *Author: George Casseus (Ndoto Studios) · License: public domain · Built: 2026-06-16* — signed by **Ndoto**.
...
- **How defended:** the repo's [LICENSE](LICENSE) (CC0 1.0) and this vow.
...
This is the **public recension** of the project's vows, prepared for the v0.3 public release from the canon file at **Recension III** (2026-05-28). The vow statements are unchanged in substance; only defense-mechanism prose that had gone stale against the v0.3 specification was re-phrased to match [`SPEC.md`](SPEC.md) — a `<head>`-resident manifest became the `<nav id="manifest">`, v0.1 field names (`sha256`, `summary`, `char_count`, `built_at`) became their v0.3 forms (`data-witness`, the manifest entry, build date in `about`-section prose — v0.3 refuses build-metadata attributes), and links into the project's internal research record became plain citations. The vows were shaped across three canon Recensions, each authorized by a written Discernment (V9 sharpened in Recension I; *The substrate is HTML* promoted to V3 in Recension II, 2026-05-22; V10 discerned inside the memory substrate it inscribes in Recension III, 2026-05-28). Those receipts, and the sealed experiments behind [`EVIDENCE.md`](EVIDENCE.md), live in the project's private record and are cited here as provenance.
